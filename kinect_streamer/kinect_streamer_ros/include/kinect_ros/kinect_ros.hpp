...
private:
    ...
    std::map<std::string, std::unique_ptr<ros::NodeHandle>> nh_keypoints;
    ...
    std::map<std::string, std::unique_ptr<image_transport::ImageTransport>> it_depths;

    std::map<std::string, std::unique_ptr<image_transport::ImageTransport>> its_registered;
    std::map<std::string, std::unique_ptr<image_transport::ImageTransport>> its_undistorted;
    ...
    std::map<std::string, image_transport::Publisher>   pub_depths;
    std::map<std::string, image_transport::Publisher>   pubs_registered;
    std::map<std::string, image_transport::Publisher>   pubs_undistorted;
    std::map<std::string, ros::Publisher>               pub_camera_info_colors;
    std::map<std::string, ros::Publisher>               pub_camera_info_irs;
    std::map<std::string, ros::Subscriber>              sub_keypoints;
...
public:
    ...
    void init_keypoint_sub(std::vector<std::string> serials);
    void send_image(std::string serial, cv::Mat img_color, cv::Mat img_ir_mono8, cv::Mat img_depth_mono8);
    void send_registered_images(std::string serial, cv::Mat img_registered, cv::Mat img_undistorted);
