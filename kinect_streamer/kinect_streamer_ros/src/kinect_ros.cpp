...
void KinectROS::init_keypoint_sub(std::vector<std::string> serials) {
    for (std::string serial : serials) {
        ...
        //sub_keypoints[serial] = nh_keypoints[serial]->subscribe<std_msgs::Float32MultiArray>(std::string("/keypoints_") + serial, 1, callback);
    }
}
...
void KinectROS::send_registered_images(std::string serial, cv::Mat img_registered, cv::Mat img_undistorted) {
    ...
    sensor_msgs::ImagePtr msg_undistorted = cv_bridge::CvImage(std_msgs::Header(), "mono8", img_undistorted).toImageMsg();
...
void KinectROS::send_image(std::string serial, cv::Mat img_color, cv::Mat img_ir_mono8, cv::Mat img_depth_mono8) {
    // Create ROS sensor messages from OpenCV Images
    ...
    sensor_msgs::ImagePtr msg_depth = cv_bridge::CvImage(std_msgs::Header(), "mono8", img_depth_mono8).toImageMsg();
