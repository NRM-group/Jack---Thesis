...
int main(int argc, char** argv) {
    ...
    std::map<std::string, std::unique_ptr<KinectROS>> kinect_ros;
...
#ifdef ENABLE_ROS
    for (std::string serial : serials) {
        kinect_ros[serial] = std::make_unique<KinectROS>(argc, argv);

#ifdef ENABLE_ROS
        kinect_ros[serial]->init_camera_pub(serials);
#endif
...
#ifdef ENABLE_ROS
        kinect_ros[serial]->init_frame_pub(serials);
        kinect_ros[serial]->init_cloud_pub(serials);
#endif
    }
    ...
    while (true) {
        ...
        for (std::string serial : serials) {
            ...
            cv::Mat img_ir_div;
            cv::Mat img_depth_div;
            img_ir_div = img_ir / 16.0;
            img_depth_div = img_depth / 20.0;
            
            cv::Mat img_ir_mono8;
            cv::Mat img_depth_mono8;
            img_ir_div.convertTo(img_ir_mono8, CV_8UC1);
            img_depth_div.convertTo(img_depth_mono8, CV_8UC1);
            ...
            cv::flip(img_depth_mono8, img_depth_mono8, 1);

#ifdef ENABLE_ROS
            kinect_ros[serial]->send_image(serial, img_color, img_ir_mono8, img_depth_mono8);
#endif
            ...
            cv::Mat img_undistorted_div;
            img_undistorted_div = img_undistorted / 20.0;

            cv::Mat img_undistorted_mono8;
            img_undistorted_div.convertTo(img_undistorted_mono8, CV_8UC1);

            cv::flip(img_undistorted_mono8, img_undistorted_mono8, 1);
            cv::flip(img_registered, img_registered, 1);

#ifdef ENABLE_ROS
            kinect_ros[serial]->send_registered_images(serial, img_registered, img_undistorted_mono8);
#endif
...
#ifdef ENABLE_ROS
            kinect_ros[serial]->send_cloud(serial, cloud_data, sizeof(cloud_data));
#endif
