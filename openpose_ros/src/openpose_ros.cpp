...
#ifdef WRITE_TO_CSV
    std::ofstream myfile;
    myfile.open("/home/medrobotics/data_20220510.csv", std::ios_base::app);
    std::ofstream kinect_data;
    kinect_data.open("/home/medrobotics/kinect_data.csv", std::ios_base::app);
#endif
    ros::spinOnce();
...
#ifdef WRITE_TO_CSV
        for (int i = 0; i <= 7; i++) {
            myfile << x_arr[i] << "," << y_arr[i] << "," << z_arr[i];
            if (i != 7) {
                myfile << ",";
            } else {
                myfile << "\n";
        ...
        if ((poseKeypoints[{0, WRST_L, 3}] > 0.8)) {
            if (abs(last_row - row_arr[WRST_L]) < 50 && abs(last_col - col_arr[WRST_L]) < 50) {
                if (last_row != -1 && last_col != -1) {
                    cv::line(img_draw, cv::Point(last_col, last_row), cv::Point(col_arr[WRST_L], row_arr[WRST_L]), cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
                }
            }
            last_row = row_arr[WRST_L];
...
void OpenPoseROS::init() {
    ...
    const auto netInputSize = op::flagsToPoint(op::String(FLAGS_net_resolution), "-1x320");
